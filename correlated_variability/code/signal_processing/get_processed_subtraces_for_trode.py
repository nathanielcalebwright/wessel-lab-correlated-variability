...
def get_processed_subtraces_for_trode(trode_name,
    ...
    '''
    For a given trode (indicated by trode_name), get an array containing
    ...
    ---------- 
    trode_name : string
        name of trode of interest (e.g., '070314_c2')
    ...
    if 'LFP' in trode_name:
        raw_traces_path = master_folder_path + '\\downsampled_LFP_traces\\%s\\'%trode_name    

    else:
        raw_traces_path = master_folder_path + '\\downsampled_traces\\%s\\'%trode_name    
    subtraces = []

    print 'processing traces for %s'%trode_name
...
#        plt.plot(trace, 'r')
        ...
        if filt_kind != 'unfiltered':
            trace = butter(trace, sampling_freq = samp_freq,
                ...
                order = 3)
        #print crit_freq, filt_kind
        #plt.figure()
        #plt.plot(trace, 'g')
        #plt.title(trode_name + ' ' + trial)
        #plt.show()
