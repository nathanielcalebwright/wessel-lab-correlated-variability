...
def plot_rel_pwr_spectra_all_cells(ax1, ax2, fontproperties,
    ...
    each cell in the dataset, for the transient and steady-state epochs.
    
    Parameters
    ----------
    ax1 : matplotlib axis
        axis displaying results for transient epoch
    ax2: matplotlib axis
        axis displaying results for steady-state epoch
...
#            rP_dict[epoch].append(numpy.mean(rP_spec_all_trials, axis = 0))
    ...
    for epoch in epochs:
        ...
        for rP in rP_all_cells:
            ax.plot(freqs, rP, color = 'k', alpha = 0.3)
        ax.plot(freqs, mean_rP, color = 'k', lw = 1.5)
